...
static_assert(Block_column_size > 0, "Block_column_size must be greater than 0"); // Check the
static_assert(Block_row_size > 0, "Block_row_size must be greater than 0");

constexpr int Total_row_blocks = (sequence_length + Block_row_size - 1) / Block_row_size;          // Tr Tr is
