...
__global__ void transposeMatrix(const float *input, float *output, int width, int height)
{
    ...
    if (x < width && y < height)  // Check if the thread is within the bounds of the input matrix
    {
        int inputIndex = y * width + x; // Calculate the index in the input matrix
        int outputIndex = x * height + y; // Calculate the index in the output
        output[outputIndex] = input[inputIndex]; // Assign the transposed value
    }
}
